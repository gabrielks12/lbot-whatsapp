...
module.exports = {
    ...
    textoParaFoto: async(texto)=>{
        try{
            var res =  await axios.get(encodeURI(`https://p7api.xyz/api/maker/ttp?texto=${texto}&apikey=dDJzbwOw`))
            return res.data.replace("data:image/webp;base64,", "")
        } catch(err){
    ...
    textoParaGif: async(texto)=>{
        try{
            var res =  await axios.get(encodeURI(`https://p7api.xyz/api/maker/attp?texto=${texto}&apikey=dDJzbwOw`))

...
module.exports = {
    ...
    textoParaFoto: async(texto)=>{
        try{
            var res = await axios.get(encodeURI(`http://localhost:5000/ttp?texto=${texto}&cor=FFA500`),
             {
                responseType: "text",
    ...
    textoParaGif: async(texto)=>{
        try{
            var res = await axios.get(encodeURI(`http://localhost:5000/attp?texto=${texto}`),
             {
                responseType: "arraybuffer",
            })

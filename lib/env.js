    ...
    verificarEnv:()=>{
        ...
        let resposta = {
            numero_dono : {
               ...
               cor_resposta: (process.env.NUMERO_DONO == "55219xxxxxxxx") ? "#d63e3e" : false
            },
            newsapi : {
              resposta: (process.env.API_NEWS_ORG == "??????") ? "A API do NEWSAPI ainda não foi configurada" : "✓ API NEWSAPI Configurada.",
              cor_resposta: (process.env.API_NEWS_ORG == "??????") ? "#d63e3e" : false
            },
            acrcloud :{
            ...
            deepai : {
              resposta: (process.env.API_DEEPAI == "??????") ? "A API do DEEPAI ainda não foi configurada" : "✓ API DEEPAI Configurada.",
              cor_resposta: (process.env.API_NEWS_ORG == "??????") ? "#d63e3e" : false
            },
          }
